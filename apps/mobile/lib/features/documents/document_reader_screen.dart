...
class _DocumentReaderScreenState extends ConsumerState<DocumentReaderScreen> {
  ...
  Future<void> _loadPage(int pageNumber, {bool quiet = false}) async {
    ...
    final target = pageCount == null
        ? pageNumber.clamp(1, 100000)
        : pageNumber.clamp(1, pageCount);
    if (!quiet && mounted) setState(() => _currentPage = target);
    try {
      final page = await ref
          .read(documentRepositoryProvider)
          .page(widget.documentId, target);
  ...
  Future<void> _goToPage(int page) async {
    final pageCount =
        (_document?['page_count'] as num?)?.toInt() ?? _pdfController.pageCount;
  ...
  Future<void> _zoom(bool increase) async {
    ...
    final next = (current + (increase ? 0.25 : -0.25)).clamp(
      _pdfController.minScale,
      _pdfController.maxScale,
    );
    await _pdfController.setZoom(
      _pdfController.viewSize.center(Offset.zero),
      next,
    );
  }
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (context) => DraggableScrollableSheet(
        ...
        builder: (context, controller) => ListView(
          ...
          children: [
            Text(
              'Page $_currentPage extracted text',
            ...
            if (ocr) ...[
              ...
              Text(
                confidence == null
                    ? 'OCR used'
                    : 'OCR used · confidence ${(confidence as num).toStringAsFixed(2)}',
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null || _document == null) {
      return Scaffold(
        appBar: AppBar(),
        body: Center(child: Text(_error ?? 'Document unavailable')),
      );
    }
            ...
            child: SizedBox(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    'Page $_currentPage${pageCount == null ? '' : ' / $pageCount'}',
                  ),
                  IconButton(
                    onPressed: pageCount != null && _currentPage >= pageCount
                        ? null
                        : () => _goToPage(_currentPage + 1),
...
class _ExtractedDocumentView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ExtractedDocumentView> createState() =>
      _ExtractedDocumentViewState();
}

class _ExtractedDocumentViewState
    extends ConsumerState<_ExtractedDocumentView> {
