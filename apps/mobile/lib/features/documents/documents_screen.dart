...
class _DocumentsScreenState extends ConsumerState<DocumentsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final documents = selected == null ? <Map<String, dynamic>>[] : await documentRepo.list(selected);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _reprocess(Map<String, dynamic> document) async {
    try {
      await ref.read(documentRepositoryProvider).reprocess(document['id'] as String);
      await _load();
    } catch (_) {
  ...
  Future<void> _delete(Map<String, dynamic> document) async {
    ...
    try {
      await ref.read(documentRepositoryProvider).delete(document['id'] as String);
      await _load();
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = _documents.where((doc) {
      ...
      return '${doc['title']} ${doc['original_filename']}'.toLowerCase().contains(query);
    }).toList();
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(onPressed: _uploading ? null : _pick, tooltip: 'Upload documents', icon: const Icon(Icons.upload_file)),
        ],
      ),
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            SearchBar(controller: _search, leading: const Icon(Icons.search), hintText: 'Search documents'),
            if (_uploading) ...[
            ...
            if (_loading)
              ...
              _EmptyState(icon: Icons.cloud_off_outlined, title: _error!, action: TextButton(onPressed: _load, child: const Text('Retry')))
            else if (selected == null)
              const _EmptyState(icon: Icons.workspaces_outline, title: 'No workspace yet')
            else if (visible.isEmpty)
              _EmptyState(icon: Icons.description_outlined, title: query.isEmpty ? 'No documents yet' : 'No matching documents', action: query.isEmpty ? FilledButton.icon(onPressed: _pick, icon: const Icon(Icons.add), label: const Text('Upload')) : null)
            else
              ...visible.map((document) => _DocumentCard(document: document, onReprocess: () => _reprocess(document), onDelete: () => _delete(document))),
          ],
        ),
      ),
      floatingActionButton: selected == null ? null : FloatingActionButton.extended(onPressed: _uploading ? null : _pick, icon: const Icon(Icons.add), label: const Text('Upload')),
    );
  }
}

class _DocumentCard extends StatelessWidget {
  const _DocumentCard({required this.document, required this.onReprocess, required this.onDelete});
    ...
    return Card(
      child: ListTile(
        ...
        trailing: PopupMenuButton<String>(
          ...
          itemBuilder: (_) => [
            if (!ready) const PopupMenuItem(value: 'reprocess', child: Text('Reprocess')),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Column(children: [Icon(icon, size: 42), const SizedBox(height: 12), Text(title), if (action != null) ...[const SizedBox(height: 12), action!]]),
      );
}
