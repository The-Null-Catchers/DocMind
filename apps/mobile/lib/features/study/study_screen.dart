...
class _StudyScreenState extends ConsumerState<StudyScreen> {
  ...
  Future<void> _reviewCard(Map<String, dynamic> card) async {
    ...
    if (synced == false && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Review saved offline. It will sync automatically when connectivity returns.',
          ),
        ),
      );
    }
  ...
  Future<void> _openQuiz(Map<String, dynamic> quizSummary) async {
    try {
      final quiz = await ref
          .read(studyRepositoryProvider)
          .quiz(quizSummary['id'] as String);
      if (!mounted) return;
      await Navigator.of(
        context,
      ).push(MaterialPageRoute(builder: (_) => _QuizScreen(quiz: quiz)));
    } catch (_) {
      if (mounted)
  ...
  Future<void> _createNote() async {
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                minLines: 4,
                maxLines: 10,
                decoration: const InputDecoration(labelText: 'Markdown note'),
              ),
            ],
          ),
        ),
    ...
    if (created == true && title.text.trim().isNotEmpty) {
      await ref
          .read(studyRepositoryProvider)
          .createNote(workspaceId, title.text.trim(), content.text);
      await _load();
    }
    title.dispose();
    content.dispose();
        ...
        body: Column(
          children: [
            if (_pendingReviews > 0)
              MaterialBanner(
                content: Text(
                  '$_pendingReviews flashcard review${_pendingReviews == 1 ? '' : 's'} waiting to sync.',
                ),
                leading: const Icon(Icons.cloud_upload_outlined),
                actions: [
                  TextButton(onPressed: _load, child: const Text('Sync')),
                ],
              ),
            Expanded(
              child: TabBarView(
    ...
    children: [
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              FilledButton(
                onPressed: cards.isEmpty ? null : () => onReview(cards.first),
      ...
      ...cards
          ...
          .map(
            (card) => Card(
              child: ListTile(
                title: Text(card['front']?.toString() ?? ''),
    ...
    if (quizzes.isEmpty)
      return const Center(
        child: Padding(
          ...
          child: Text(
            'No quizzes yet. Generate one from selected documents on the web or API.',
          ),
        ),
      );
    return ListView(
          ...
          .map(
            (quiz) => Card(
              child: ListTile(
                ...
                subtitle: Text(
                  '${quiz['difficulty'] ?? 'medium'} · ${quiz['language'] ?? 'en'}',
                ),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => onOpen(quiz),
              ),
                    ...
                    child: ExpansionTile(
                      ...
                      children: [
                        Align(
                          ...
                          child: MarkdownBody(
                            data: note['content_markdown']?.toString() ?? '',
                          ),
                        ),
                      ],
                    ),
...
class _QuizScreenState extends ConsumerState<_QuizScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    final questions = (widget.quiz['questions'] as List? ?? const [])
      ...
      body: ListView(
        ...
        children: [
          ...
          if (_result == null)
            ...
            Text(
              'Score: ${(_result!['score'] as num).toStringAsFixed(0)}%',
              style: Theme.of(
                context,
            ...
            ...((_result!['results'] as List).map((raw) {
              ...
              return Card(
                child: ListTile(
                  ...
                  title: Text(item['correct'] == true ? 'Correct' : 'Review'),
